...
We believe we can formalize and store open and reusable performance metrics to be reused by anyone running a method comparison (aka benchmark). We also aim to automate decisions, to reduce implicit biases.
...
We aim to provide tested software implementations for commonly used metrics (e.g. TPR), annotated with _metric nutrition labels_, akin to [dataset nutrition labels](https://datanutrition.org/). Based on the metric nature, including syntax and semantics, input and output interfaces are clear and reusable across languages and interpreters.
...
Early stage, now installable end to end. The schema, the seed metric registry, the MCDA primitives, an ontology-aware entry point, four sensitivity primitives, a cross-dataset aggregator, and a simulated test suite are in place. The decision module is complete: five aggregations (SAW, TOPSIS, VIKOR, PROMETHEE II, COMET), five objective and subjective weighting schemes, and a re-analysis of the real Duo 2018 clustering benchmark. A user-facing layer is in place: a CSV loader, the `beam.rank` procedural API, a self-contained HTML report, a reproducibility manifest, a declarative `beam.yaml` runner, and a `beam` command-line interface. The heterogeneity module covers four models, each wrapping an R package via a subprocess: the mixed-effects variance decomposition (`mixed_effects`, lme4, with a glmmTMB beta engine for bounded metrics), the Bradley-Terry trees (`bradley_terry_tree`, psychotree), and the Plackett-Luce model on full rankings (`plackett_luce`, PlackettLuce).
...
- Twenty-seven seed metric cards across five domains, showing the registry is domain neutral: clustering and efficiency (`ari`, `runtime`, `nmi`, `peak_memory`, `accuracy`, `f1_score`, `silhouette`, `shannon_entropy_diff`, `nclust_deviation`), forecasting (`smape`, `mase`), the illustrative transportation metrics (`speed`, `cost`, `co2`), the single-cell integration and spatial metrics from OpenProblems (the scIB set `asw_batch`, `asw_label`, `cell_cycle_conservation`, `graph_connectivity`, `hvg_overlap`, `isolated_label_asw`, `isolated_label_f1`, `kbet`, `ilisi`, `clisi`, `pcr`, plus `correlation`), and a clinical-prediction calibration metric (`calibration_slope`, a `target_value` metric whose ideal is exactly 1).
...
- `beam.heterogeneity`: the method-dataset heterogeneity diagnostics, each wrapping an R package in a one-shot subprocess. `mixed_effects` fits a mixed-effects model on one metric (`score ~ method + (1 | dataset)`, adding a `(1 | dataset:method)` term when a cell has replicates) in R's lme4 and returns the per-method marginal means, the variance components, the dataset ICC, the interaction or residual share, and the largest-residual outlier cells. It quantifies how much of the score variance is a method-by-dataset interaction, the formal complement to `leave_one_dataset_out`. `bradley_terry_tree` turns each dataset into pairwise method comparisons on one metric (`paired_comparisons`), fits a Bradley-Terry tree in R's psychotree, and splits the datasets by their features so each leaf has its own ranking; the report carries the splits with their parameter-stability p-values, the per-leaf worths, the leaf assignment, the global flat ranking, and `reversed_leaves` (the subgroups where the pooled top method does not hold). This is the interpretable answer to the "against one method fits all" critique. `mixed_effects` also takes `engine="glmmtmb"` with `family="beta"` to model a metric bounded in (0, 1) on the link scale, the documented extension of the Gaussian lme4 fit. `plackett_luce` fits a Plackett-Luce model on per-dataset rankings of the methods (`rankings_from_matrix` builds the rankings), returning a worth per method that sums to one with reference-free quasi-standard-errors; it generalizes the Bradley-Terry strengths from pairwise wins to full orderings. `source_variance_decomposition` lifts the mixed-effects fit to several benchmarks at once (`score ~ method + (1 | benchmark) + (1 | benchmark:dataset) + (1 | method:benchmark)`, lme4, dataset nested in benchmark) and reports the method-by-benchmark variance share, the disagreement attributable to the benchmarker rather than the method. `r_available`, `bttree_available`, `glmmtmb_available` and `plackett_luce_available` gate the calls; the R toolchain comes from the `envs/heterogeneity.yml` conda environment. See [docs/explanations/heterogeneity-mixed-effects.md](docs/explanations/heterogeneity-mixed-effects.md), [docs/explanations/heterogeneity-bradley-terry.md](docs/explanations/heterogeneity-bradley-terry.md) and [docs/explanations/full-rankings-and-bounded-metrics.md](docs/explanations/full-rankings-and-bounded-metrics.md).
- `beam.scenarios`: canonical simulated benchmark scenarios with documented ground truth (`random` with anti-correlated trade-offs, `dominant`, `ties`, `odd_dataset` where one method is best on most datasets but a different method is best on one odd dataset), plus two normalization-failure examples where the top-ranked method under plain min-max differs from the one under the card defaults. Used by the test suite and by a simulated scenarios vignette. `beam.datasets.load_duo2018` loads the real Duo et al. 2018 clustering benchmark (14 methods by 12 datasets by 4 metrics) vendored under `src/beam/data/`. `beam.datasets.load_m4` loads a small results table derived from the M4 forecasting competition (25 methods by 6 frequency bands by 2 metrics), reduced from the GPL-3 M4comp2018 data by `src/beam/data/reduce_m4.R`. `beam.datasets.load_openproblems` loads two OpenProblems tasks (batch integration, 19 methods by 6 datasets by 13 metrics; spatially variable genes, 14 methods by 50 datasets by 1 metric) from bundled CC-BY-4.0 derived tables, with `load_openproblems_svg_features` supplying the spatial dataset features for the Bradley-Terry tree. `beam.datasets.load_integration_benchmarks` and `load_integration_published_ranks` harmonize three single-cell integration benchmarks (scIB, OpenProblems and Tran et al. 2020) on the shared scIB metric family (ARI, ASW, kBET, LISI) for five common methods, the input to the cross-benchmark meta-analysis.
...
- Six worked vignettes under `examples/`: the Duo 2018 walkthrough on the real published data, a simulated scenarios report that runs the full report layout on every canonical scenario, a cross-domain transportation example that exercises every aggregation and weighting scheme where terrain coverage is partial, an M4 forecasting example on a large real non-bio benchmark, an OpenProblems example that runs MCDA on the batch integration task and a Bradley-Terry tree on the 50-dataset spatially variable genes task (where the top method depends on the spatial assay), and a cross-benchmark meta-analysis that re-ranks three single-cell integration benchmarks under one consistent rule and decomposes their disagreement into method, dataset and benchmarker components. The first five read metric semantics from the registry.
...
examples/
  ...
  cross_benchmark/cross_benchmark.qmd  Meta-analysis of three integration benchmarks
docs/
