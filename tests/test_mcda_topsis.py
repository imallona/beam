...
def test_topsis_dominator_reaches_one():
    ...
    normalized = np.array([[1.0, 1.0], [0.0, 0.0]])
    weights = np.array([0.5, 0.5])
    result = topsis(normalized, weights)
...
def test_topsis_identical_tools_get_half():
    ...
    normalized = np.array([[0.5, 0.5], [0.5, 0.5], [0.5, 0.5]])
    weights = np.array([0.5, 0.5])
    result = topsis(normalized, weights)
...
def test_topsis_weight_tilt_picks_the_metric_specialist():
    """If most weight is on metric 0, the tool that is best on metric 0 ranks first."""
    normalized = np.array(
        [
            [1.0, 0.0],
            [0.5, 0.5],
    ...
    result = topsis(normalized, np.array([0.9, 0.1]))
...
def test_topsis_invariant_under_uniform_weight_scaling():
    ...
    normalized = rng.random((6, 4))
    weights = rng.random(4)
    base = topsis(normalized, weights)
    scaled = topsis(normalized, weights * 3.7)
...
def test_topsis_zero_weight_metric_is_ignored():
    ...
    # With weight only on column 0, tool 0 ranks first.
    result = topsis(normalized, np.array([1.0, 0.0]))
    assert result[0] > result[1]
    # Now flip the weight onto column 1; tool 1 ranks first.
    result = topsis(normalized, np.array([0.0, 1.0]))
