...
The deep-learning methods scIB scores alongside the classical five, on the same datasets and metrics. Loaded by `beam.datasets.load_scib_integration_families`, which stacks them with the classical block from `scib2022_metrics.csv` into one method by dataset by metric tensor with a per-method family label, used by the cross-benchmark vignette for a controlled family contrast (only the method differs, not the data) via `dataset_discrimination` and `difficulty_concordance`.

Six methods (scVI, scANVI, scGen, DESC, SAUCIE, trVAE) on the five real datasets (immune_cell_hum, immune_cell_hum_mou, lung_atlas, mouse_brain, pancreas) and the four metrics shared with the classical block (ARI, ASW, kBET, LISI), all scIB's unscaled output, higher is better. One representative integration variant per method, the embedding output where the method offers one (scvi_embed, scanvi_embed, scgen_full, desc_embed, saucie_embed, trvae_embed), matching the variant choice of the classical block. scGen is scored on four of the five datasets and trVAE on three; a method not scored on a dataset is absent and read as `numpy.nan`, never imputed.

Reduced from the same source as `scib2022_metrics.csv`, the `theislab/scib-reproducibility` file `data/metrics.csv`, by `reduce_scib_dl.py` in this folder, at the pinned commit `5f9c08e213c714db70f96144acd4179f9481c3d2` (the last change to that file, 2021-10-08). It keeps the unscaled rows for the five real datasets, the four metric columns (ARI_cluster/label, ASW_label, kBET, iLISI), and the deep-learning variants, emitting one row per feature space so the loader averages them, exactly as for the classical block. Repository code is MIT and the article is CC-BY 4.0; cite Luecken et al. (Nature Methods 2022, DOI 10.1038/s41592-021-01336-8). beam's role is reanalysis under one consistent rule.
