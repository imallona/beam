...
Because no mode runs on every terrain, the analysis is done one terrain at a time. On each terrain the infeasible modes are dropped (their cells are NaN) and the MCDA pipeline runs over the modes that actually run there. This is the example-level NaN handling: no imputation and no pooling across terrains, just the modes that were measured on the terrain in question. The helper `feasible_submatrix` does the row dropping and returns a dense, NaN-free score matrix ready for `run`.

Speed and cost span orders of magnitude across modes, so they use `log_min_max`. CO2 has true zeros (walking, cycling), so it uses `rank`, which is defined when a column carries hard zeros.
...
MEREC is left out of the table above. It takes the logarithm of the normalized scores, so it cannot accept a column that carries a hard zero, and the card normalization here (`rank` on CO2) produces such a zero. Run with a zero-free normalization instead. Under z-score normalization MEREC reads the removal effect of each metric and weights cost and CO2 above speed, so it lands on a cheap mode at the top, the same lesson as the other data-driven weightings.
