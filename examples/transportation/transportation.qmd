...
`pairwise_transitivity` confirms it from the pairwise side: the four ground modes have a fixed speed order, so the pairwise majorities are transitive and one order is consistent with them.

```{python}
from beam.mcda import pairwise_superiority, pairwise_transitivity
from matplotlib.colors import ListedColormap

ground_sup = pairwise_superiority(block, "higher_is_better", method_names=block_modes)
ground_trans = pairwise_transitivity(ground_sup)
print(f"transitive: {ground_trans.is_transitive}; circular triads: {ground_trans.n_circular_triads}")
if ground_trans.consistent_order is not None:
    print("consistent order:", [block_modes[i] for i in ground_trans.consistent_order])
```

The dominance matrix below orders the modes by how many others they outperform. Here it is a clean upper triangle: every faster mode outperforms every slower one, with no red cell below the diagonal, so the relation is transitive. This is the opposite of the Duo ARI case, where the pairwise majorities carry a cycle.

```{python}
dom = ground_trans.dominance
order = list(np.argsort(-dom.sum(axis=1), kind="stable"))
labels = [block_modes[i] for i in order]
grid = np.zeros((ground_trans.n_methods, ground_trans.n_methods))
...
ax.set_ylabel("mode (row), ordered by pairwise wins")
ax.set_title("Pairwise dominance on ground modes by speed (transitive)")
fig.tight_layout()
plt.show()
```
