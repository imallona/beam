...
[`card_data_consistency`](../../docs/explanations/card-data-consistency.md) checks the scores against what the 12 cards declare, before any normalization. The scIB metrics are scaled to [0, 1] on the platform, so the audit gives a clean bill here; it would name any metric whose values fell outside its declared range.
...
The two rules disagree on the top-ranked method. This is the same point the [M4](../m4/m4.qmd) and Duo vignettes make: a benchmark recommendation is partly a decision about how the criteria are combined. beam makes that decision explicit; a single leaderboard number does not show it.
...
`pairwise_transitivity` reads the same pairwise relation and reports whether one order is [consistent with it](../../docs/explanations/transitivity.md).
