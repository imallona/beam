...
Splitting the methods into a deep-learning family and a classical one separates two kinds of hardness. `difficulty_concordance` correlates the per-family difficulty profiles across the datasets. A high concordance places the hardness in the data; a low one places it in the method family.
...
The two benchmarks differ because their datasets are hard for different reasons. On OpenProblems the hardness is dataset complexity, and the families agree (Spearman about 0.89): a dataset hard for classical methods is hard for the deep-learning ones. On Shen the hardness is annotation quality, and the families agree weakly (about 0.32): degraded labels affect only the methods that use them. On the hardest units the deep-learning family scores below the classical one; on the easy units it scores above. Dataset complexity is shared across families; a benchmark that stresses what one family depends on is hard only for that family, which a single cross-method ranking would hide.
