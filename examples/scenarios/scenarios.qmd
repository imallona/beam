...
labels = [odd.method_names[i] for i in order]
...
ax.set_title("Pairwise dominance (red marks a cycle)")
fig.tight_layout()
plt.show()
```
...
A mixed-effects model on this metric reports a small dataset intercept and a large residual. With one observation per cell the residual is the method-by-dataset interaction confounded with noise, and here it dominates because the methods reorder between the two regimes rather than the datasets differing in difficulty. The fits need the R toolchain, so the chunks run only when it is available.
...
A blind analysis fixes the weighting and the aggregation before the method names are known. `beam.blind` hides the names; `beam.unblind` restores them. The order is unchanged.
