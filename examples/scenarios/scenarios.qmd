...
The dominant scenario's top method has a rank span of zero and leads every combination, as built: with no dataset axis and a method that wins everywhere, no modeling choice can move it. In the random scenario the top method's rank moves with the choice, and the variance falls on the weighting and aggregation rather than on any data, because there is no data axis to carry it.
