...
for name in ["dominant", "random"]:
    ...
    top = s.method_names[rs.headline_tool]
    ...
    print(f"           top method {top} ranks first in {rs.headline_top_fraction * 100:.0f}% "
          f"of combinations, rank span {rs.headline_rank_span}")
```

The dominant scenario's top method has a rank span of zero and is rank 1 in every combination, as built: with no dataset axis and a method that scores highest everywhere, no modeling choice can move it. In the random scenario the top method's rank moves with the choice, and the variance falls on the weighting and aggregation rather than on any data, because there is no data axis to carry it.
