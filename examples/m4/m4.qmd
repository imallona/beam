...
```{python}
labels = ["band\n(dataset)", "weighting", "aggregation", "interactions"]
...
ax.set_title("What moves the M4 ranking")
fig.tight_layout()
plt.show()
```
...
ax.set_title("Specification curve on M4 (weighting x aggregation x band)")
fig.tight_layout()
plt.show()
```
...
A blind analysis fixes the pipeline before the method names are known, so the weighting and the metric set cannot be chosen to favor a method expected to lead. `beam.blind` hides the names and shuffles the rows; `beam.unblind` restores them. The ranking is unchanged, and the seal fingerprint is recorded in the manifest.
...
labels = [m4.method_names[i] for i in order]
...
ax.set_title("Pairwise dominance on M4 sMAPE (red marks a cycle)")
fig.tight_layout()
plt.show()
```
