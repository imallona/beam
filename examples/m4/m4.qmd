...
On M4 the band intercept takes most of the sMAPE variance: the bands differ mostly in how hard they are to forecast for every method alike, which is the opposite of the transportation example, where the terrain decides which mode is best. A Bradley-Terry tree on the six bands, with the seasonal period as the splitting feature, has too few datasets to find a stable split, the same small-sample limit the Duo benchmark hits and the reason the OpenProblems spatial task (50 datasets) is where a split appears.
