...
The pooled ranking averages over the six frequency bands. `dataset_concordance` ranks the methods within each band and compares every pair of per-band orderings with Kendall tau-b. A high mean says the pooled ranking stands in for the individual bands; a low mean says it does not. It needs no replicates and assumes nothing about the bands being interchangeable.
...
M4 is the contrast to Duo: the bands disagree more, which is the strong method-by-band interaction the rank-sensitivity section also reads. The agreement matrix shows the bands that pull together and the ones that stand apart.
