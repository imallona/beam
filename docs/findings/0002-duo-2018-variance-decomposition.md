...
The variance decomposition is the formal complement to the leave-one-dataset-out check in [findings 0001](0001-duo-2018-mcda.md). Leave-one-dataset-out showed Seurat's composite rank is stable to dropping any single dataset; this shows why that stability is plausible on ARI, since two thirds of the ARI variance is a shared dataset shift rather than methods trading places. The residual share of 0.29 is the headroom for genuine method-by-dataset interaction plus noise, and the outlier cells point to where it sits: the two weakest methods on a few specific datasets, not a broad reshuffling of the strong methods. This is consistent with the critical-difference reading in findings 0001, where the upper-middle methods were not separable. The honest limit is identifiability: at one run per cell beam cannot say how much of that 0.29 is interaction and how much is noise; a Gaussian likelihood on a bounded metric is also an approximation near 0 and 1. Both are addressed by the documented extensions, multi-run inputs and a glmmTMB beta family (PLAN Phase 4).
