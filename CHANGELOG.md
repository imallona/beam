...
- `beam.datasets.load_scib_integration_families`: the scIB deep-learning methods (scVI, scANVI, scGen, DESC, SAUCIE, trVAE) extracted from the same source as the classical five, scored on identical datasets and metrics, so the families can be contrasted on the same data. The cross-benchmark vignette uses it for a controlled `difficulty_concordance` check; scores vendored as scib2022_dl_metrics.csv via reduce_scib_dl.py.
- `beam.mcda.dataset_concordance` and `DatasetConcordanceReport`: ranks the methods within each dataset and reports the dataset-by-dataset Kendall tau-b agreement, the most idiosyncratic dataset, a grouping of mutually consistent datasets, and the method-by-dataset cells that depart most from a method's own typical rank. It measures the heterogeneity that makes a single pooled ranking partial, needs no replicates, and assumes no exchangeability among the datasets; `beam.rank` attaches it for a tensor input, plotted by `beam.plot.dataset_concordance` and `beam.plot.dataset_struggle`, wrapped in R as `beam_dataset_concordance`; docs/explanations/dataset-concordance.md.
...
- `beam.plot.rank_sensitivity_by_tool`: the per-method companion to `rank_sensitivity`, one stacked bar splitting each method's rank variance across the weighting, the aggregation and the dataset, so a method whose rank depends on the data is told apart from one that depends on a choice. Reachable from R through `beam_plot(rs, "rank_sensitivity_by_tool")`; shown in the duo2018, m4, openproblems, cell-type-annotation, transportation, scenarios and cross-benchmark vignettes.
...
- The SMAA rank-acceptability colorbar in the funky heatmap read the wrong way round: the legend mapped rank 1 to the dark end while the bars drew rank 1 bright, so a top method looked like a low rank. The colorbar now uses the reversed colormap and is flipped so rank 1 is bright and at the top.
